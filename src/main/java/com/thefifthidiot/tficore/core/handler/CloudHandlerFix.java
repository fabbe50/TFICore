...
public class CloudHandlerFix extends IRenderHandler{
    private static final ResourceLocation CLOUD_TEXTURE = new ResourceLocation("textures/environment/clouds.png");
    public static CloudHandlerFix INSTANCE = new CloudHandlerFix();
    ...
    public void render(float partTicks, WorldClient world, Minecraft minecraft) {
        if (world.provider.isSurfaceWorld()) {
            if (minecraft.gameSettings.clouds == 2) {
                ...
                Vec3d vec3d = Configs.cloudColorRed == 1.0f && Configs.cloudColorGreen == 1.0f && Configs.cloudColorBlue == 1.0f ? world.getCloudColour(partTicks) : getCloudColorBody(partTicks, minecraft);
    ...
    public void renderFancyClouds(float partTicks, WorldClient world, Minecraft minecraft) {
        ...
        GlStateManager.enableBlend();
        GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
        Vec3d vec3 = Configs.cloudColorRed == 1.0f && Configs.cloudColorGreen == 1.0f && Configs.cloudColorBlue == 1.0f ? world.getCloudColour(partTicks) : getCloudColorBody(partTicks, minecraft);
    ...
    @SideOnly(Side.CLIENT)
    public Vec3d getCloudColorBody(float partialTicks, Minecraft mc)
    {
        float f = mc.world.getCelestialAngle(partialTicks);
        float f1 = MathHelper.cos(f * ((float)Math.PI * 2F)) * 2.0F + 0.5F;
        f1 = MathHelper.clamp(f1, 0.0F, 1.0F);
        float f2 = red;
        float f3 = green;
        float f4 = blue;
        float f5 = mc.world.getRainStrength(partialTicks);

        if (f5 > 0.0F)
        {
            float f6 = (f2 * 0.3F + f3 * 0.59F + f4 * 0.11F) * 0.6F;
            float f7 = 1.0F - f5 * 0.95F;
            if (f2 > f3 && f2 > f4) {f2 = f2 * f7 + f6 * (1.0F - f7);}
            if (f3 > f4 && f3 > f2) {f3 = f3 * f7 + f6 * (1.0F - f7);}
            if (f4 > f2 && f4 > f3) {f4 = f4 * f7 + f6 * (1.0F - f7);}
        }

        if (f2 > f3 && f2 > f4) {f2 = f2 * (f1 * 0.9F + 0.1F);}
        if (f3 > f4 && f3 > f2) {f3 = f3 * (f1 * 0.9F + 0.1F);}
        if (f4 > f2 && f4 > f3) {f4 = f4 * (f1 * 0.85F + 0.15F);}
        float f9 = mc.world.getThunderStrength(partialTicks);

        if (f9 > 0.0F)
        {
            float f10 = (f2 * 0.3F + f3 * 0.59F + f4 * 0.11F) * 0.2F;
            float f8 = 1.0F - f9 * 0.95F;
            if (f2 > f3 && f2 > f4) {f2 = f2 * f8 + f10 * (1.0F - f8);}
            if (f3 > f4 && f3 > f2) {f3 = f3 * f8 + f10 * (1.0F - f8);}
            if (f4 > f2 && f4 > f3) {f4 = f4 * f8 + f10 * (1.0F - f8);}
        }
